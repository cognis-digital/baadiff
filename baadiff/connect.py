"""Native cognis-connect emit for baadiff — forward findings to any platform.

Maps baadiff's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    baadiff ... --format json | baadiff-emit --to stix
    baadiff-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['type'] = rec.get('type', 'gap')
        out['description'] = rec.get('message', 'No description provided')
